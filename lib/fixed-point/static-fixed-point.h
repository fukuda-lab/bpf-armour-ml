...
{
    struct fixed_point result;
    result.number = ((int64_t)first->number * second->number) >> 8;
    result.q = 8;
    return result;
}
...
{
    struct fixed_point result;
    result.number = ((u_int64_t)first->number << 8) / second->number;
    result.q = 8;
    return result;
}
...
// x is the adding number, n is the number of elements, m is the mean, m2 is the variance
// struct fixed_point __always_inline variance(struct fixed_point *x, struct fixed_point *n, struct fixed_point *m, struct fixed_point *m2)
...
int fixed_sqrt(struct fixed_point *number, struct fixed_point *result)
{
    ...
    unsigned int t = (unsigned int)(63 - number->q) / 2;
...
int get_variance(struct fixed_point *m2, int n, struct fixed_point *result)
{
    if (!m2 || !result || n <= 1)
    {
